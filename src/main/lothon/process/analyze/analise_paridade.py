...
class AnaliseParidade(AbstractAnalyze):
    ...
    def execute(self, loteria: Loteria) -> int:
        ...
        for key, value in enumerate(cp.ultimas_paridades_repetidas):
            percent: float = cp.ultimas_paridades_percentos[key]
            output += f"\t {formatd(key,2)} pares:  {formatf(percent,'6.2')}%  ...  " \
                      f"#{formatd(value)}\n"
        logger.debug(f"{nmlot}: Concursos que repetiram a ultima paridade: {output}")
