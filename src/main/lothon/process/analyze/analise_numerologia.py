...
class AnaliseNumerologia(AbstractAnalyze):
    ...
    def execute(self, payload: Loteria) -> int:
        ...
        numelogias: list[Optional[Bola]] = self.new_list_bolas(qtd_items)
        ...
        for bola in numelogias:
            ...
            output += f"\t     {bola.id_bola}:       " \
                      ...
                      f"{formatf(bola.gmean_atraso,'6.1')}    " \
                      f"{formatf(bola.median_atraso,'6.1')}   " \
                      f"{formatf(bola.varia_atraso,'9.1')}          " \
                      f"{formatf(bola.stdev_atraso,'6.1')} \n"
