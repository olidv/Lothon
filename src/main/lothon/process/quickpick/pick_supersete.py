...
class PickSuperSete(AbstractQuickPick):
    """
    ...
    def __init__(self, loteria: Loteria):
        super().__init__("Geracao de Palpites para Super Sete", loteria)
    ...
    def gerar_palpite(self, max_recorrencias: int,
        ...
        while True:
            ...
            jogo_sorteado = cb.sortear_palpite_digito(self.loteria.qtd_bolas_sorteio)
    ...
    def execute(self, qtd_palpites: int, concursos: list[Concurso] = None) -> list[tuple]:
        ...
        palpites: list[tuple[int, ...]] = []  # aqui estao todos os palpites

        # efetua o sorteio do(s) jogo(s) com o numero de dezenas requerido:
        for _ in range(0, qtd_palpites):
            jogo_sorteado: tuple[int, ...] = self.gerar_palpite(max_recorrencias, palpites)
